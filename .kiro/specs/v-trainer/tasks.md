...
- [ ] 2. Set up Room database schema for local caching
...
- [ ] 3. Define core domain models
  ...
  - [~] 3.2 Create enum classes (MuscleGroup, MediaType, Difficulty, Equipment, SyncStatus, RecordType)
    - Define all enum values from design
    - _Requirements: 2.1_
  
  - [~] 3.3 Create mapper functions between domain models and Room entities
    - Implement bidirectional mapping with JSON serialization for nested objects
    - _Requirements: 12.1_
  
  - [~] 3.4 Write property test for WorkoutPlan serialization
...
- [ ] 4. Implement training log validation logic
  - [~] 4.1 Create validation functions for TrainingLog data
  ...
  - [~] 4.2 Write property test for validation rejecting invalid data
    - **Property 2: Training Log Validation Rejects Invalid Data**
    - **Validates: Requirements 20.1, 20.2, 20.3, 20.4, 20.5, 20.6**
    - Use fast-check to generate invalid TrainingLogs
  
  - [~] 4.3 Write property test for validation accepting valid data
...
- [ ] 6. Implement volume calculation and personal record detection
  - [~] 6.1 Create volume calculator utility
    - Calculate total volume as sum of (weight × reps) for all sets
    - _Requirements: 7.3_
  
  - [~] 6.2 Write property test for volume calculation correctness
  ...
  - [~] 6.3 Create personal record detector utility
    - Compare exercise performance against user's history
    - Detect max weight records
    - Detect max volume records
    - _Requirements: 8.1, 8.2, 8.3_
  
  - [~] 6.4 Write property test for max weight record detection
    - **Property 5: Personal Record Detection for Max Weight**
    - **Validates: Requirements 8.1, 8.2, 8.4**
  
  - [~] 6.5 Write property test for max volume record detection
    - **Property 6: Personal Record Detection for Max Volume**
    - **Validates: Requirements 8.1, 8.3, 8.4**

- [ ] 7. Implement calculateProgress Cloud Function
  - [~] 7.1 Create Firestore onCreate trigger for training_logs collection
  ...
  - [~] 7.2 Write unit tests for calculateProgress function
...
- [ ] 8. Implement sendWorkoutReminder Cloud Function
  - [~] 8.1 Create scheduled function for workout reminders
  ...
  - [~] 8.2 Write unit tests for reminder function
...
- [~] 10. Checkpoint - Deploy and test backend
...
- [ ] 11. Implement WorkoutRepository with offline-first strategy
  - [~] 11.1 Create WorkoutRepository interface and implementation
  ...
  - [~] 11.2 Write property test for workout plan creation preserving exercise count
